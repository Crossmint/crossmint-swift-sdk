...
struct TransferView2: View {
    ...
    var body: some View {
        ...
        .onChange(of: balances, { _, newValue in
            if let newValue {
                availableTokens = newValue.tokens
...
//        .crossmintNonCustodialSigner(CrossmintSDK.shared)
    ...
    private func triggerTransaction() async {
        ...
        do {
            async let waitedsummary = try wallet.send(
                recipientWallet,
                "base-sepolia:\(selectedToken)",
                amount
            )
//            async let no = try wallet.send(
//                recipientWallet,
//                "base-sepolia:\(selectedToken)",
//                amount
//            )
//            await print("2nd one: \(try no.transactionID)")
            let summary = try await waitedsummary
            ...
            default:
                await MainActor.run {
                    alertViewModel.show(title: "Transaction Failed", message: "Error: \(error)")
                }
            }
        }
    }
