...
public enum SignerConfig: Sendable {
    ...
    ///
    /// The first value is the address of the external wallet.
    ///
    /// To add the external wallet as a signer, pass this case to ``Wallet/addSigner(_:)``.
    /// This call does not use `onSign`.
    /// To sign with the external wallet, pass this case with `onSign` to ``Wallet/useSigner(_:)``.
    ///
    /// The SDK calls `onSign` each time the wallet needs an approval from the external wallet.
