...
actor DataDogLoggerProvider: LoggerProvider {
    ...
    private func write(level: LogLevel, message: String, attributes: [String: Encodable]?) {
        guard DataDogConfig.trackingConsent != .notGranted else { return }
    ...
    func flush() {
        ...
        switch DataDogConfig.trackingConsent {
        case .notGranted:
            batchQueue.removeAll()
            return
        case .pending:
            // Hold the batch until the user resolves consent one way or the other.
            return
        case .granted:
            break
        }
